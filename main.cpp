...
int round(double x) {   //the clion IDE dont like this function,run on vcode with -std=c++17
...
int main() {
    ...
    TestCase("Test MyStruct operators", cerr)
            ...
            .check_function([](const MyStruct& s){return s.myNum();}, MyStruct(5), 5) // Here there is a bug. //run on vcode with -std=c++17
            .print();
}
